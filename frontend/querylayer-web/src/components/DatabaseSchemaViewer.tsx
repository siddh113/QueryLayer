...
function typeColor(dataType: string): string {
  return TYPE_COLORS[dataType.toLowerCase()] ?? "var(--text-faint)";
}
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>{table.name}</div>
            {table.entityName && (
              <div className="text-[11px]" style={{ color: "var(--text-muted)" }}>entity: {table.entityName}</div>
            )}
          </div>
        </div>
        {!table.exists ? (
      ...
      {table.exists && table.columns.length > 0 ? (
        ...
        >
          <div className="grid grid-cols-3 px-4 py-2 text-[11px] font-medium uppercase tracking-wider"
            style={{ background: "var(--bg-hover)", color: "var(--text-muted)", borderBottom: "1px solid var(--border-subtle)" }}
          >
            <span>Column</span>
            <span>Type</span>
            <span>Nullable</span>
          ...
          {table.columns.map((col, i) => (
            <div
              ...
              style={{
                borderBottom: i < table.columns.length - 1 ? "1px solid var(--border-subtle)" : "none",
                background: i % 2 === 0 ? "transparent" : "var(--bg-hover)",
              }}
            >
              <span className="font-mono font-medium" style={{ color: "var(--text-secondary)" }}>{col.columnName}</span>
              <span className="font-mono text-[11px]" style={{ color: typeColor(col.dataType) }}>
                {col.dataType}{col.maxLength ? `(${col.maxLength})` : ""}
              </span>
              <span style={{ color: col.isNullable ? "var(--success-text)" : "var(--text-muted)" }}>
        ...
        <div className="text-xs text-center py-3" style={{ color: "var(--text-muted)" }}>No columns found</div>
      ) : (
    ...
    <div className="space-y-5">
        ...
        >
          {loading ? (
            <span className="animate-spin w-3 h-3 rounded-full"
              style={{ border: "1.5px solid var(--border)", borderTopColor: "currentColor" }}
            />
          ) : (
      ...
      {!loading && !error && data && data.tables.length === 0 && (
        <div className="rounded-2xl glass-card p-8 text-center">
          ...
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--accent-cyan)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <ellipse cx="12" cy="5" rx="9" ry="3" />
              <path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3" />
              <path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5" />
            </svg>
